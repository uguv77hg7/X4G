...
.pg.on .metric{animation:slideUp .4s ease both}
.pg.on .metric:nth-child(1){animation-delay:.05s}
.pg.on .metric:nth-child(2){animation-delay:.1s}
.pg.on .metric:nth-child(3){animation-delay:.15s}
.pg.on .metric:nth-child(4){animation-delay:.2s}

.pg.on .card{animation:slideUp .35s ease both}
.pg.on .g2 .card:nth-child(2){animation-delay:.1s}
.pg.on .g3 .card:nth-child(2){animation-delay:.1s}

.pg.on .cfg-card{animation:slideUp .3s ease both}
.pg.on .cfg-card:nth-child(1){animation-delay:.03s}
.pg.on .cfg-card:nth-child(2){animation-delay:.06s}
.pg.on .cfg-card:nth-child(3){animation-delay:.09s}
.pg.on .cfg-card:nth-child(4){animation-delay:.12s}
.pg.on .cfg-card:nth-child(5){animation-delay:.15s}
.pg.on .cfg-card:nth-child(6){animation-delay:.18s}

.pg.on .conn-card-v2{animation:scaleIn .35s ease both}
.pg.on .conn-card-v2:nth-child(1){animation-delay:.05s}
.pg.on .conn-card-v2:nth-child(2){animation-delay:.1s}
.pg.on .conn-card-v2:nth-child(3){animation-delay:.15s}
.pg.on .conn-card-v2:nth-child(4){animation-delay:.2s}

.pg.on .sub-card{animation:slideUp .35s ease both}
.pg.on .sub-card:nth-child(1){animation-delay:.04s}
.pg.on .sub-card:nth-child(2){animation-delay:.08s}
.pg.on .sub-card:nth-child(3){animation-delay:.12s}

.pg.on .traf-hero .traf-main-stat{animation:slideUp .4s ease both}
.pg.on .traf-hero .traf-mini{animation:slideUp .35s ease both}
.pg.on .traf-hero .traf-mini:nth-child(2){animation-delay:.08s}
.pg.on .traf-hero .traf-mini:nth-child(3){animation-delay:.14s}
.pg.on .traf-hero .traf-mini:nth-child(4){animation-delay:.2s}

.pg.on .traf-chart-card{animation:slideUp .4s ease .15s both}

.pg.on .conn-hero-tile{animation:scaleIn .35s ease both}
.pg.on .conn-hero-tile:nth-child(1){animation-delay:.05s}
.pg.on .conn-hero-tile:nth-child(2){animation-delay:.1s}
.pg.on .conn-hero-tile:nth-child(3){animation-delay:.15s}
.pg.on .conn-hero-tile:nth-child(4){animation-delay:.2s}

.pg.on .create-panel{animation:slideUp .4s ease both}
.pg.on .srv-panel{animation:slideUp .4s ease both}
.pg.on .pw-panel{animation:slideUp .4s ease .08s both}

.pg.on .log-item{animation:slideRight .3s ease both}
.pg.on .log-item:nth-child(1){animation-delay:.03s}
.pg.on .log-item:nth-child(2){animation-delay:.06s}
.pg.on .log-item:nth-child(3){animation-delay:.09s}
.pg.on .log-item:nth-child(4){animation-delay:.12s}
.pg.on .log-item:nth-child(5){animation-delay:.15s}

.pg.on .topbar{animation:slideUp .3s ease both}
.pg.on .links-toolbar{animation:slideUp .3s ease .05s both}
.pg.on .bulk-bar{animation:slideUp .3s ease .1s both}

#particles-canvas{position:fixed;inset:0;z-index:0;pointer-events:none}

.cfg-card,.conn-card-v2,.metric,.traf-mini,.traf-main-stat,.sub-card,.stat-card,.cfgdash-item{
...
.create-panel:hover{animation:borderGlow 2s ease infinite}
.vless-box:hover{animation:borderGlow 2s ease infinite}

/* ══════ FEATURE 3: Extra Themes ══════ */
[data-theme="dark"][data-color="ocean"]{--accent:#3B82F6;--accent2:#60A5FA;--accent-d:rgba(59,130,246,0.12);--card-b:rgba(59,130,246,0.13);--card-bh:rgba(59,130,246,0.28)}
[data-theme="dark"][data-color="emerald"]{--accent:#10B981;--accent2:#34D399;--accent-d:rgba(16,185,129,0.12);--card-b:rgba(16,185,129,0.13);--card-bh:rgba(16,185,129,0.28)}
[data-theme="dark"][data-color="ruby"]{--accent:#EF4444;--accent2:#F87171;--accent-d:rgba(239,68,68,0.12);--card-b:rgba(239,68,68,0.13);--card-bh:rgba(239,68,68,0.28)}
[data-theme="dark"][data-color="amethyst"]{--accent:#8B5CF6;--accent2:#A78BFA;--accent-d:rgba(139,92,246,0.12);--card-b:rgba(139,92,246,0.13);--card-bh:rgba(139,92,246,0.28)}
[data-theme="dark"][data-color="gold"]{--accent:#D4AF37;--accent2:#E8C84A;--accent-d:rgba(212,175,55,0.12);--card-b:rgba(212,175,55,0.13);--card-bh:rgba(212,175,55,0.28)}
[data-theme="light"][data-color="ocean"]{--accent:#2563EB;--accent2:#1D4ED8;--accent-d:rgba(37,99,235,0.08)}
[data-theme="light"][data-color="emerald"]{--accent:#059669;--accent2:#047857;--accent-d:rgba(5,150,105,0.08)}
[data-theme="light"][data-color="ruby"]{--accent:#DC2626;--accent2:#B91C1C;--accent-d:rgba(220,38,38,0.08)}
[data-theme="light"][data-color="amethyst"]{--accent:#7C3AED;--accent2:#6D28D9;--accent-d:rgba(124,58,237,0.08)}
[data-theme="light"][data-color="gold"]{--accent:#B8860B;--accent2:#996515;--accent-d:rgba(184,134,11,0.08)}
.theme-selector{display:flex;gap:5px;flex-wrap:wrap;margin-bottom:8px}
.theme-dot{width:22px;height:22px;border-radius:50%;border:2px solid var(--card-b);cursor:pointer;transition:.18s;position:relative}
.theme-dot:hover{transform:scale(1.15)}
.theme-dot.active{border-color:var(--t1);box-shadow:0 0 0 2px var(--accent)}
.theme-dot::after{content:'';position:absolute;inset:3px;border-radius:50%}

/* ══════ FEATURE 4: Zen Mode ══════ */
body.zen-mode .sidebar{display:none}
body.zen-mode .main{margin-right:0;padding:20px}
body.zen-mode .metrics{display:none}
body.zen-mode .topbar .tb-right .badge{display:none}
.zen-exit{display:none;position:fixed;bottom:20px;left:20px;z-index:1000;background:var(--accent);color:#fff;border:none;border-radius:50%;width:48px;height:48px;font-size:20px;cursor:pointer;box-shadow:0 4px 16px rgba(0,0,0,.3)}
body.zen-mode .zen-exit{display:flex;align-items:center;justify-content:center}

/* ══════ FEATURE 5: Font Size ══════ */
.font-size-ctrl{display:flex;align-items:center;gap:8px;margin-top:6px;padding:8px;background:var(--accent-d);border:1px solid var(--card-b);border-radius:9px}
.font-size-ctrl button{width:28px;height:28px;border-radius:6px;border:1px solid var(--card-b);background:var(--card);color:var(--t2);font-size:14px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:.15s}
.font-size-ctrl button:hover{background:var(--accent-d);color:var(--t1)}
.font-size-ctrl span{font-size:11px;color:var(--t2);font-weight:600;min-width:32px;text-align:center}

/* ══════ FEATURE 6: Offline Banner ══════ */
.offline-banner{display:none;position:fixed;top:0;left:0;right:0;z-index:9999;background:var(--red);color:#fff;text-align:center;padding:8px;font-size:12px;font-weight:700}
body.offline .offline-banner{display:block}

/* ══════ FEATURE 10: One-Hand Mode ══════ */
body.one-hand .main{padding-bottom:60vh}
body.one-hand .sidebar{top:auto;bottom:0;height:auto;max-height:50vh;border-radius:16px 16px 0 0}
.one-hand-btn{position:fixed;bottom:20px;right:20px;z-index:1000;width:44px;height:44px;border-radius:50%;background:var(--accent-d);border:1px solid var(--card-b);color:var(--t2);font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:.15s}
.one-hand-btn:hover{background:var(--accent-d);color:var(--accent)}

/* ══════ FEATURE 11: Lock Overlay ══════ */
#lock-overlay.show{display:flex}

/* ══════ FEATURE 9: Better Mobile Sidebar ══════ */
@media(max-width:1050px){
  .sidebar{backdrop-filter:blur(20px);background:rgba(17,17,24,0.95)}
  .nav-it{min-height:48px;padding:12px 14px}
  [data-theme="light"] .sidebar{background:rgba(240,237,224,0.95)}
}
</style>
</head>
<body>
<div class="offline-banner">⚠ شما آفلاین هستید — اتصال اینترنت را بررسی کنید</div>
...
<aside class="sidebar" id="sb">
  ...
  <div class="nav-wrap">
    ...
    <div class="nav-it" data-pg="converter"><i class="ti ti-transform"></i> تبدیل لینک</div>
  </div>
  <div class="sb-foot">
    <div class="theme-selector" id="theme-selector">
      <div class="theme-dot" data-color="gold" onclick="setColorTheme('gold')" title="Gold" style="background:#D4AF37"></div>
      <div class="theme-dot" data-color="ocean" onclick="setColorTheme('ocean')" title="Ocean" style="background:#3B82F6"></div>
      <div class="theme-dot" data-color="emerald" onclick="setColorTheme('emerald')" title="Emerald" style="background:#10B981"></div>
      <div class="theme-dot" data-color="ruby" onclick="setColorTheme('ruby')" title="Ruby" style="background:#EF4444"></div>
      <div class="theme-dot" data-color="amethyst" onclick="setColorTheme('amethyst')" title="Amethyst" style="background:#8B5CF6"></div>
    </div>
    <button class="theme-btn" onclick="toggleTheme()"><i class="ti ti-moon" id="theme-icon"></i> <span id="theme-label">تم روشن</span></button>
    <button class="theme-btn" onclick="toggleZenMode()"><i class="ti ti-focus-2"></i> <span>حالت زِن</span></button>
...
<section class="pg" id="pg-testws">
  ...
  <div class="card" style="max-width:660px">
    ...
    <div class="form-row" style="margin-bottom:12px">
      ...
      <button class="btn btn-pur" onclick="pingTest()"><i class="ti ti-clock"></i> پینگ</button>
    </div>
...
<section class="pg" id="pg-converter">
  <div class="topbar">
    <div><div class="tb-title"><i class="ti ti-transform"></i> تبدیل لینک</div><div class="tb-sub">تبدیل بین فرمت‌های مختلف VPN لینک</div></div>
  </div>
  <div class="card" style="max-width:800px">
    <div class="card-title"><i class="ti ti-arrows-left-right"></i> مبدل لینک VPN</div>
    <div class="cl" style="margin-top:0;margin-bottom:14px"><i class="ti ti-info-circle"></i><span>لینک VLESS، VMess یا Trojan را در کادر زیر وارد کنید و دکمه تبدیل را بزنید تا اجزای آن نمایش داده شود.</span></div>
    <div style="margin-bottom:14px">
      <textarea class="fi" id="converter-input" style="width:100%;min-height:120px;resize:vertical;font-family:ui-monospace,monospace;font-size:11px;line-height:1.8" placeholder="vless://uuid@host:port?params#label&#10;vmess://base64...&#10;trojan://password@host:port?params#label"></textarea>
    </div>
    <div style="display:flex;gap:8px;margin-bottom:14px">
      <button class="btn btn-p" onclick="convertLink()"><i class="ti ti-transform"></i> تبدیل</button>
      <button class="btn btn-o" onclick="document.getElementById('converter-input').value='';document.getElementById('converter-output').innerHTML=''"><i class="ti ti-x"></i> پاک کردن</button>
    </div>
    <div id="converter-output"></div>
  </div>
</section>
<div class="zen-exit" onclick="toggleZenMode()"><i class="ti ti-x"></i></div>
<button class="one-hand-btn" onclick="toggleOneHand()" title="حالت یک‌دستی"><i class="ti ti-hand-click"></i></button>
<div id="lock-overlay">
  <i class="ti ti-lock" style="font-size:48px;color:var(--accent)"></i>
  <div style="font-size:16px;font-weight:700;color:var(--t1)">صفحه قفل شد</div>
  <button class="btn btn-p" onclick="unlockScreen()" style="padding:12px 30px">باز کردن قفل</button>
</div>
</main>
<script>
...
function animateCounter(el,target,duration){
  ...
  var start=0;var startTime=performance.now();
...
async function fetchStats(){
  try{
    ...
    var ec=document.getElementById('m-conns');
    if(ec)animateCounter(ec,d.active_connections);
    document.getElementById('conns-nb').textContent=d.active_connections;
    var et=document.getElementById('m-traffic');
    if(et)animateCounter(et,d.total_traffic_mb);
    document.getElementById('m-traffic-suffix').innerHTML='<span class="m-unit">MB</span>';
    ...
    var ee=document.getElementById('m-errs');
    if(ee)animateCounter(ee,d.total_errors||0);
    ...
    if(d.protocol_stats){
      const labels=Object.keys(d.protocol_stats);
      const vals=Object.values(d.protocol_stats);
      const colors=['#D4AF37','#50C878','#C9A0FF','#FFB020'];
      if(ch2){ch2.data.labels=labels;ch2.data.datasets[0].data=vals;ch2.data.datasets[0].backgroundColor=colors.slice(0,labels.length);ch2.update();}
    }
    if(d.hourly){
...
document.addEventListener('DOMContentLoaded',async()=>{
      ...
      for(let i=0;i<particles.length;i++){
        for(let j=i+1;j<particles.length;j++){
          ...
          if(dist<150){
            ...
            ctx.strokeStyle=`rgba(212,175,55,${.06*(1-dist/150)})`;
  ...
  (function(){
    const selectors='.cfg-card,.conn-card-v2,.metric,.traf-mini,.traf-main-stat,.sub-card,.stat-card,.cfgdash-item';
    document.addEventListener('mousemove',e=>{
      document.querySelectorAll(selectors).forEach(card=>{
        const rect=card.getBoundingClientRect();
        const x=e.clientX-rect.left,y=e.clientY-rect.top;
        if(x<0||x>rect.width||y<0||y>rect.height){
          card.style.transform='';card.style.transition='transform .3s ease';
          return;
        }
...
// ══════ FEATURE 3: Theme Color System ══════
var currentColor=localStorage.getItem('x4g-theme-name')||'gold';
function setColorTheme(c){
  currentColor=c;
  localStorage.setItem('x4g-theme-name',c);
  document.documentElement.setAttribute('data-color',c);
  document.querySelectorAll('.theme-dot').forEach(d=>{d.classList.toggle('active',d.dataset.color===c)});
}
function initColorTheme(){setColorTheme(currentColor)}
initColorTheme();
...
function toggleZenMode(){
  ...
  localStorage.setItem('x4g-zen',isZen?'1':'0');
}
if(localStorage.getItem('x4g-zen')==='1')document.body.classList.add('zen-mode');

// ══════ FEATURE 5: Font Size Controls ══════
var currentFontSize=parseInt(localStorage.getItem('x4g-fontsize'))||14;
function setFontSize(size){document.documentElement.style.fontSize=size+'px';localStorage.setItem('x4g-fontsize',size);document.getElementById('font-size-val').textContent=size+'px'}
function changeFontSize(dir){
  var next=currentFontSize+dir;
  if(next<12||next>18)return;
...
// ══════ FEATURE 8: Touch Gestures ══════
var touchStartX=0;
document.addEventListener('touchstart',function(e){touchStartX=e.touches[0].clientX});
document.addEventListener('touchend',function(e){
  var dx=e.changedTouches[0].clientX-touchStartX;
  if(Math.abs(dx)<80)return;
  var pages=['overview','links','cfgdash','traffic','connections','security','logs','errors','testws','settings','support','converter'];
  var current=document.querySelector('.pg.on');
  if(!current)return;
  var curId=current.id.replace('pg-','');
  var idx=pages.indexOf(curId);
  if(dx<0&&idx<pages.length-1)navTo(pages[idx+1]);
  else if(dx>0&&idx>0)navTo(pages[idx-1]);
});

// ══════ FEATURE 10: One-Hand Mode ══════
function toggleOneHand(){
  document.body.classList.toggle('one-hand');
  localStorage.setItem('x4g-onehand',document.body.classList.contains('one-hand')?'1':'0');
}
if(localStorage.getItem('x4g-onehand')==='1')document.body.classList.add('one-hand');

// ══════ FEATURE 11: Auto-Lock ══════
var lockTimer;
function resetLockTimer(){
  clearTimeout(lockTimer);
  lockTimer=setTimeout(function(){
    var lo=document.getElementById('lock-overlay');
    if(lo)lo.classList.add('show');
  },300000);
}
['mousemove','keydown','click','scroll','touchstart'].forEach(function(e){document.addEventListener(e,resetLockTimer)});
...
// ══════ FEATURE 12: Link Converter ══════
function convertLink(){
  var input=document.getElementById('converter-input').value.trim();
  var out=document.getElementById('converter-output');
  if(!input){out.innerHTML='<div class="cl"><i class="ti ti-alert-circle"></i><span>لیسکی وارد کنید</span></div>';return}
  var lines=input.split('\n').filter(function(l){return l.trim()});
  var html='';
  lines.forEach(function(line){
    line=line.trim();
    if(line.startsWith('vless://')){
      html+=parseVless(line);
    }else if(line.startsWith('vmess://')){
      html+=parseVmess(line);
    }else if(line.startsWith('trojan://')){
      html+=parseTrojan(line);
    }else{
      html+='<div class="card" style="margin-bottom:10px"><div class="card-title"><i class="ti ti-alert-circle"></i> فرمت ناشناخته</div><div class="cl"><i class="ti ti-info-circle"></i><span>فرمت لینک تشخیص داده نشد</span></div></div>';
    }
  });
  out.innerHTML=html;
}
function parseVless(link){
  try{
    var rest=link.substring(7);
    var hashIdx=rest.lastIndexOf('#');
    var label=hashIdx>-1?decodeURIComponent(rest.substring(hashIdx+1)):'—';
    var mainPart=hashIdx>-1?rest.substring(0,hashIdx):rest;
    var qIdx=mainPart.indexOf('?');
    var hostPort=qIdx>-1?mainPart.substring(0,qIdx):mainPart;
    var params=qIdx>-1?mainPart.substring(qIdx+1):'';
    var atIdx=hostPort.lastIndexOf('@');
    var uuid=atIdx>-1?hostPort.substring(0,atIdx):hostPort;
    var addrPort=atIdx>-1?hostPort.substring(atIdx+1):'';
    var pIdx=addrPort.lastIndexOf(':');
    var host=pIdx>-1?addrPort.substring(0,pIdx):addrPort;
    var port=pIdx>-1?addrPort.substring(pIdx+1):'443';
    var paramMap={};if(params){params.split('&').forEach(function(p){var kv=p.split('=');paramMap[decodeURIComponent(kv[0])]=decodeURIComponent(kv[1]||'')})}
    var items=[
      ['نوع','VLESS'],['UUID',uuid],[['آدرس',host],['پورت',port]],
      ['tls',paramMap.security||'none'],['sni',paramMap.sni||''],['fp',paramMap.fingerprint||''],
      ['network',paramMap.type||'ws'],['path',paramMap.path||paramMap.serviceName||''],
      ['عنوان',label]
    ];
    return renderParsed('VLESS',link,items);
  }catch(e){return renderError('VLESS')}
}
function parseVmess(link){
  try{
    ...
    var items=[
      ['نوع','VMess'],['آدرس',json.add||''],['پورت',json.port||''],
      ['UUID',json.id||''],['تعداد',json.aid||'0'],
      ['tls',json.tls||''],['net',json.net||''],['path',json.path||''],
      ['sni',json.sni||''],['fp',json.fp||''],['عنوان',json.ps||'—']
    ];
    return renderParsed('VMess',link,items);
  }catch(e){return renderError('VMess')}
}
function parseTrojan(link){
  try{
    ...
    var params=qIdx>-1?mainPart.substring(qIdx+1):'';
    var atIdx=hostPort.lastIndexOf('@');
    var password=atIdx>-1?hostPort.substring(0,atIdx):hostPort;
    var addrPort=atIdx>-1?hostPort.substring(atIdx+1):'';
    var pIdx=addrPort.lastIndexOf(':');
    var host=pIdx>-1?addrPort.substring(0,pIdx):addrPort;
    var port=pIdx>-1?addrPort.substring(pIdx+1):'443';
    var paramMap={};if(params){params.split('&').forEach(function(p){var kv=p.split('=');paramMap[decodeURIComponent(kv[0])]=decodeURIComponent(kv[1]||'')})}
    var items=[
      ['نوع','Trojan'],['رمزعبور',password],[['آدرس',host],['پورت',port]],
      ['sni',paramMap.sni||''],['fp',paramMap.fingerprint||''],['network',paramMap.type||''],
      ['عنوان',label]
    ];
    return renderParsed('Trojan',link,items);
  }catch(e){return renderError('Trojan')}
}
function renderParsed(type,link,items){
  var rows=items.map(function(item){
    if(Array.isArray(item[0])){return '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">'+item.map(function(i){return '<div><div class="m-label">'+esc(i[0])+'</div><div style="font-size:13px;font-weight:700;color:var(--t1);word-break:break-all">'+esc(i[1])+'</div></div>'}).join('')+'</div>'}
    if(typeof item==='string')return '';
    return '<div class="sr"><span class="sr-k"><i class="ti ti-chevron-left"></i> '+esc(item[0])+'</span><span class="sr-v" style="font-size:10.5px;word-break:break-all">'+esc(item[1])+'</span></div>';
  }).join('');
  return '<div class="card" style="margin-bottom:10px"><div class="card-title"><i class="ti ti-circle-check" style="color:var(--green)"></i> '+type+' — تبدیل شد</div>'+rows+'</div>';
}
function renderError(type){return '<div class="card" style="margin-bottom:10px"><div class="card-title"><i class="ti ti-alert-circle" style="color:var(--red)"></i> خطا در پردازش '+type+'</div><div class="cl amber"><i class="ti ti-info-circle"></i><span>لینک '+type+' معتبر نیست یا فرمت آن نادرست است.</span></div></div>'}
...
def get_public_page_html(uuid_key: str) -> str:
    """صفحه پابلیکساب v4 — با ۱۵ ویژگی جدید"""
...
:root{{--bg:#060a14;--bg2:#0a1020;--bg3:#0d1428;--card:#0c1326;--card-b:rgba(96,148,246,0.12);--card-bh:rgba(96,148,246,0.28);--accent:#3B7CF6;--accent2:#6EA3FF;--accent-d:rgba(59,124,246,0.1);--green:#1FB87E;--green-bg:rgba(31,184,126,0.1);--green-t:#3FD79C;--red:#EF4444;--red-bg:rgba(239,68,68,0.1);--red-t:#FB8585;--amber:#F2A33D;--amber-bg:rgba(242,163,61,0.1);--amber-t:#F9C988;--purple:#9D7BF0;--purple-bg:rgba(157,123,240,0.1);--purple-t:#BCA4F7;--t1:#EFF4FF;--t2:#8AA0C4;--t3:#48577A;--radius:18px;--shadow:0 12px 40px rgba(0,0,0,0.45);--serif:'Vazirmatn',sans-serif}}
[data-theme="light"]{{--bg:#F0F3FA;--card:#FFFFFF;--card-b:rgba(59,124,246,0.14);--accent:#2E63D6;--accent2:#1E4CB8;--accent-d:rgba(46,99,214,0.08);--green:#0E9A6A;--green-bg:rgba(14,154,106,0.08);--green-t:#0A7553;--red:#DC2626;--red-bg:rgba(220,38,38,0.08);--red-t:#A51E1E;--amber:#C97A12;--amber-bg:rgba(201,122,18,0.08);--amber-t:#8F5A0C;--purple:#7350D6;--purple-bg:rgba(115,80,214,0.08);--purple-t:#5A3CAD;--t1:#101A30;--t2:#48577A;--t3:#8694B0;--shadow:0 12px 36px rgba(20,40,90,0.12)}}
...
.aurora-blob{{position:absolute;border-radius:50%;filter:blur(100px);opacity:.12;animation:auroraFloat 12s ease-in-out infinite alternate}}
.ab1{{width:500px;height:400px;background:linear-gradient(135deg,#3B7CF6,#6EA3FF);top:-15%;right:-10%;animation-duration:10s}}
.ab2{{width:400px;height:350px;background:linear-gradient(135deg,#1FB87E,#3FD79C);bottom:-15%;left:-8%;animation-delay:3s;animation-duration:14s}}
.ab3{{width:350px;height:300px;background:linear-gradient(135deg,#9D7BF0,#BCA4F7);top:35%;left:25%;animation-delay:6s;animation-duration:16s;opacity:.08}}
@keyframes auroraFloat{{0%{{transform:translate(0,0) scale(1)}}33%{{transform:translate(40px,-30px) scale(1.1)}}66%{{transform:translate(-20px,50px) scale(.95)}}100%{{transform:translate(30px,20px) scale(1.05)}}}}
...
.preset-bar{{display:flex;gap:5px;align-items:center}}
.preset-dot{{width:22px;height:22px;border-radius:50%;cursor:pointer;border:2px solid transparent;transition:.2s}}
.preset-dot:hover{{transform:scale(1.2)}}
.preset-dot.active{{border-color:var(--t1);box-shadow:0 0 6px rgba(255,255,255,.2)}}
.pd-blue{{background:#3B7CF6}}.pd-green{{background:#10B981}}.pd-purple{{background:#8B5CF6}}.pd-gold{{background:#D4AF37}}
.countdown-bar{{background:linear-gradient(135deg,var(--accent),#2952C8);border-radius:16px;padding:14px 20px;margin-bottom:18px;display:flex;align-items:center;gap:14px;box-shadow:0 8px 24px rgba(59,124,246,.25);flex-wrap:wrap}}
.countdown-icon{{width:40px;height:40px;border-radius:12px;background:rgba(255,255,255,.18);display:flex;align-items:center;justify-content:center;font-size:20px;color:#fff;flex-shrink:0}}
.countdown-text{{flex:1;min-width:140px}}
.countdown-label{{font-size:10px;color:rgba(255,255,255,.75);font-weight:600;text-transform:uppercase;letter-spacing:.06em}}
.countdown-val{{font-size:22px;font-weight:800;color:#fff;display:flex;gap:6px;margin-top:2px}}
.cd-unit{{font-size:11px;font-weight:600;opacity:.8;align-self:flex-end;margin-bottom:2px}}
.cd-sep{{font-size:18px;opacity:.5;align-self:flex-end;margin-bottom:1px}}
.countdown-warn{{font-size:11px;color:#FCD34D;font-weight:700}}
...
.chart-section{{background:var(--card);border:1px solid var(--card-b);border-radius:20px;padding:20px 22px;margin-bottom:18px;box-shadow:var(--shadow)}}
.chart-head{{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px}}
.chart-title{{font-size:13px;font-weight:800;color:var(--t1);display:flex;align-items:center;gap:7px}}
.chart-title i{{color:var(--accent);font-size:16px}}
.chart-wrap{{height:200px;position:relative}}
.calendar-section{{background:var(--card);border:1px solid var(--card-b);border-radius:20px;padding:20px 22px;margin-bottom:18px;box-shadow:var(--shadow)}}
.cal-head{{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px}}
.cal-title{{font-size:13px;font-weight:800;color:var(--t1);display:flex;align-items:center;gap:7px}}
.cal-title i{{color:var(--green);font-size:16px}}
.cal-month{{font-size:11px;color:var(--t3);font-weight:600}}
.cal-grid{{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}}
.cal-day{{aspect-ratio:1;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:var(--t3);background:rgba(96,148,246,.04)}}
.cal-day.active{{background:var(--green-bg);color:var(--green-t)}}
.cal-day.today{{border:1.5px solid var(--accent)}}
.cal-legend{{display:flex;gap:12px;margin-top:10px;font-size:9px;color:var(--t3);align-items:center}}
...
.cfg-badges{{display:flex;gap:5px;flex-wrap:wrap;margin-top:6px;align-items:center}}
...
.health-row{{display:flex;align-items:center;gap:8px;margin-top:8px;flex-wrap:wrap}}
.health-pill{{font-size:9px;font-weight:700;padding:3px 8px;border-radius:20px;display:flex;align-items:center;gap:4px}}
.hp-ok{{background:var(--green-bg);color:var(--green-t)}}.hp-err{{background:var(--red-bg);color:var(--red-t)}}.hp-chk{{background:var(--amber-bg);color:var(--amber-t)}}
.ping-val{{font-size:9.5px;font-weight:700;padding:3px 8px;border-radius:20px}}
.ping-green{{background:var(--green-bg);color:var(--green-t)}}.ping-amber{{background:var(--amber-bg);color:var(--amber-t)}}.ping-red{{background:var(--red-bg);color:var(--red-t)}}
...
.cfg-status.ok{{background:var(--green-bg);color:var(--green-t)}}.cfg-status.no{{background:var(--red-bg);color:var(--red-t)}}
...
.cfg-actions{{display:flex;gap:7px;flex-wrap:wrap;margin-top:11px;align-items:center}}
...
.btn-tg{{background:#0098e6;color:#fff}}.btn-tg:hover{{background:#0080cc}}
.btn-wa{{background:#25D366;color:#fff}}.btn-wa:hover{{background:#1ebe57}}
...
.lock-card{{background:var(--card);border:1px solid var(--card-b);border-radius:26px;text-align:center;max-width:380px;width:100%;box-shadow:var(--shadow);overflow:hidden}}
.lock-banner{{background:linear-gradient(150deg,rgba(59,124,246,.16),rgba(59,124,246,.02) 70%);padding:38px 30px 26px}}
...
.lock-inp{{width:100%;padding:13px 44px;border-radius:13px;border:1px solid var(--card-b);background:rgba(0,0,0,.2);color:var(--t1);font-family:inherit;font-size:14px;outline:none;text-align:center;letter-spacing:.14em;transition:.18s}}
...
.rate-warn{{background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);border-radius:11px;padding:10px 14px;margin-bottom:12px;font-size:11.5px;color:var(--red-t);display:none;align-items:center;gap:7px}}
.rate-warn.show{{display:flex}}
...
.qr-box{{background:linear-gradient(155deg,var(--bg3),var(--card) 60%);border:1px solid var(--card-b);border-radius:22px;padding:26px;text-align:center;max-width:340px;width:100%;box-shadow:var(--shadow);position:relative;overflow:hidden}}
.qr-box::before{{content:'';position:absolute;top:-40px;right:-40px;width:150px;height:150px;background:radial-gradient(circle,rgba(59,124,246,.15),transparent 70%);pointer-events:none}}
.qr-title{{font-size:13.5px;font-weight:800;margin-bottom:16px;color:var(--t1);position:relative;z-index:1}}
.qr-img{{border-radius:14px;overflow:hidden;margin-bottom:15px;position:relative;z-index:1}}
.qr-img img{{width:100%;display:block;background:#fff;padding:10px;border-radius:14px}}
.qr-logo-overlay{{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:40px;height:40px;border-radius:50%;background:#fff;padding:4px;box-shadow:0 2px 8px rgba(0,0,0,.15);z-index:2;border:2px solid var(--card)}}
.qr-logo-overlay img{{width:100%;height:100%;border-radius:50%}}
.timeout-modal{{display:none;position:fixed;inset:0;background:rgba(0,0,0,.8);z-index:700;align-items:center;justify-content:center;backdrop-filter:blur(8px);padding:20px}}
.timeout-modal.open{{display:flex}}
.timeout-card{{background:var(--card);border:1px solid var(--card-b);border-radius:22px;padding:32px;text-align:center;max-width:360px;width:100%;box-shadow:var(--shadow)}}
.timeout-icon{{width:56px;height:56px;border-radius:16px;background:var(--amber-bg);display:flex;align-items:center;justify-content:center;margin:0 auto 16px}}
.timeout-icon i{{font-size:24px;color:var(--amber)}}
.timeout-title{{font-size:16px;font-weight:800;color:var(--t1);margin-bottom:8px}}
.timeout-sub{{font-size:12px;color:var(--t3);line-height:1.7;margin-bottom:16px}}
.timeout-timer{{font-size:28px;font-weight:800;color:var(--amber);font-family:ui-monospace,monospace;margin-bottom:16px}}
...
@media(max-width:520px){{.stats-bar{{grid-template-columns:1fr 1fr}}.sub-name{{font-size:19px}}.copy-all-bar{{flex-direction:column;align-items:stretch}}.wrap{{padding:16px 12px 50px}}.countdown-val{{font-size:17px}}}}
</style>
</head>
<body>
<div class="aurora-wrap"><div class="aurora-blob ab1"></div><div class="aurora-blob ab2"></div><div class="aurora-blob ab3"></div></div>
<div class="bg-fx"></div><div class="grid-fx"></div>
<div class="toast" id="toast"></div>
<div class="timeout-modal" id="timeout-modal"><div class="timeout-card"><div class="timeout-icon"><i class="ti ti-clock-off"></i></div><div class="timeout-title">اتصال منقضی می‌شود</div><div class="timeout-sub">صفحه به دلیل عدم فعالیت قفل خواهد شد</div><div class="timeout-timer" id="timeout-countdown">60</div><button class="btn btn-p" style="width:100%;justify-content:center;padding:12px" onclick="extendSession()">ادامه اتصال</button></div></div>
<div class="qr-modal" id="qr-modal" onclick="this.classList.remove('open')"><div class="qr-box" onclick="event.stopPropagation()"><div class="qr-title" id="qr-label">QR Code</div><div class="qr-img" style="position:relative"><img id="qr-img" src="" alt="QR"><div class="qr-logo-overlay"><img src="data:image/png;base64,{LOGO_B64}" alt="X4G"></div></div><button class="btn btn-g" style="width:100%;justify-content:center" onclick="document.getElementById('qr-modal').classList.remove('open')"><i class="ti ti-x"></i> بستن</button></div></div>
<div class="wrap">
  <div class="top">
    <div class="brand"><div class="brand-img"><img src="data:image/png;base64,{LOGO_B64}" alt="X4G"></div><div><div class="brand-name">X4G</div><div class="brand-sub">v9.8</div></div></div>
    <div class="top-actions">
      <div class="preset-bar"><div class="preset-dot pd-blue active" onclick="setPreset('')" title="Default"></div><div class="preset-dot pd-green" onclick="setPreset('emerald')" title="Emerald"></div><div class="preset-dot pd-purple" onclick="setPreset('royal')" title="Royal"></div><div class="preset-dot pd-gold" onclick="setPreset('sunset')" title="Sunset"></div></div>
      <button class="icon-btn" id="lang-toggle" onclick="toggleLang()" title="Language"><i class="ti ti-language"></i></button>
      <button class="icon-btn" id="theme-toggle" onclick="toggleTheme()" title="Theme"><i class="ti ti-sun" id="theme-icon"></i></button>
    </div>
  </div>
...
let savedPw='';
const TR={{fa:{{loading:'در حال بارگذاری...',error:'خطا',lockTitle:'گروه رمزدار',lockSub:'رمز رو وارد کنید',lockBtn:'ورود',lockErr:'رمز اشتباه',lockFooter:'اتصال رمزنگاری شده',group:'گروه دسترسی',lastUp:'آخرین بروزرسانی',copySub:'کپی ساب',qrAll:'QR کل',copyAll:'کپی همه',copyAllSub:'کپی یکجا',activeCfgs:'کانفیگ فعال',liveConns:'اتصالات زنده',online:'آنلاین',totalUse:'کل مصرف',allCfgs:'همه',cfgs:'کانفیگ‌ها',showL:'نمایش لینک',hideL:'پنهان کردن',copyL:'کپی لینک',active:'فعال',inactive:'غیرفعال',conn:'اتصال',used:'مصرف',quota:'سهمیه',expIn:'زمان باقی‌مانده',expired:'منقضی!',expSoon:'نزدیک انقضا!',chartT:'نمودار ۷ روز',calT:'تقویم ۳۰ روز',shareAll:'اشتراک‌گذاری',copied:'کپی شد ✓',noCfg:'کانفیگی نیست',cfgCopied:'کپی شد ✓',rateWarn:'تلاش زیاد. صبر کنید.',rateLock:'قفل ۳۰ ثانیه',cont:'ادامه',close:'بستن'}},en:{{loading:'Loading...',error:'Error',lockTitle:'Password Protected',lockSub:'Enter password',lockBtn:'Enter',lockErr:'Wrong password',lockFooter:'Encrypted connection',group:'Access Group',lastUp:'Last updated',copySub:'Copy sub',qrAll:'QR All',copyAll:'Copy All',copyAllSub:'Copy all at once',activeCfgs:'Active configs',liveConns:'Live connections',online:'Online',totalUse:'Total usage',allCfgs:'All',cfgs:'Configs',showL:'Show link',hideL:'Hide link',copyL:'Copy link',active:'Active',inactive:'Inactive',conn:'connections',used:'used',quota:'quota',expIn:'Time Remaining',expired:'Expired!',expSoon:'Expiring Soon!',chartT:'7-Day Chart',calT:'30-Day Calendar',shareAll:'Share',copied:'Copied ✓',noCfg:'No configs',cfgCopied:'Copied ✓',rateWarn:'Too many attempts. Wait.',rateLock:'Locked 30s',cont:'Continue',close:'Close'}}}};
let curLang=localStorage.getItem('x4g-pub-lang')||'fa';
function t(k){{return TR[curLang][k]||k}}
function toggleLang(){{curLang=curLang==='fa'?'en':'fa';localStorage.setItem('x4g-pub-lang',curLang);document.documentElement.lang=curLang;document.documentElement.dir=curLang==='fa'?'rtl':'ltr'}}
...
applyTheme(isDark);
let curPreset=localStorage.getItem('x4g-pub-preset')||'';
function setPreset(p){{curPreset=p;localStorage.setItem('x4g-pub-preset',p);document.documentElement.setAttribute('data-preset',p);document.querySelectorAll('.preset-dot').forEach(d=>d.classList.remove('active'));const sel=p?'.pd-'+p.substring(0,p.length>5?5:p.length):'.pd-blue';const el=document.querySelector(sel);if(el)el.classList.add('active')}}
setPreset(curPreset);
function toast(m,t2){{const e=document.getElementById('toast');e.textContent=m;e.className='toast show'+(t2?' '+t2:'');setTimeout(()=>e.classList.remove('show'),2400)}}
function esc(s){{return String(s||'').replace(/[&<>"']/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}}[c]))}}
function fmtB(b){{if(!b)return '0 B';if(b<1024)return b+' B';if(b<1048576)return (b/1024).toFixed(1)+' KB';if(b<1073741824)return (b/1048576).toFixed(2)+' MB';return (b/1073741824).toFixed(2)+' GB'}}
function toFa(n){{return String(n).replace(/\\d/g,d=>{{return '۰۱۲۳۴۵۶۷۸۹'[parseInt(d)]}})}}
function protoChip(p){{return p&&p.startsWith('xhttp')?'<span class="proto-chip pc-xhttp"><i class="ti ti-bolt"></i> XHTTP</span>':'<span class="proto-chip pc-ws">VLESS·WS</span>'}}
function showQR(lb,ln){{document.getElementById('qr-label').textContent=lb;document.getElementById('qr-img').src='https://api.qrserver.com/v1/create-qr-code/?size=260x260&data='+encodeURIComponent(ln);document.getElementById('qr-modal').classList.add('open')}}
function toggleLink(i){{const w=document.getElementById('vw-'+i);const b=document.getElementById('vt-'+i);const o=w.classList.toggle('open');b.classList.toggle('open',o);b.querySelector('.ltl span').textContent=o?t('hideL'):t('showL')}}
let countdownIV=null;
function startCountdown(ea){{if(!ea)return;const tgt=new Date(ea).getTime();if(isNaN(tgt))return;function upd(){{const d=tgt-Date.now();if(d<=0){{document.getElementById('cd-bar').innerHTML='<div class="countdown-icon"><i class="ti ti-alert-triangle"></i></div><div class="countdown-text"><div class="countdown-label">'+t('expired')+'</div></div>';clearInterval(countdownIV);return}}const dd=Math.floor(d/864e5),hh=Math.floor(d%864e5/36e5),mm=Math.floor(d%36e5/6e4),ss=Math.floor(d%6e4/1e3);document.getElementById('cd-vals').innerHTML=(dd>0?'<span>'+toFa(dd)+'<span class="cd-unit"> روز</span></span><span class="cd-sep">:</span>':'')+'<span>'+toFa(String(hh).padStart(2,'0'))+'<span class="cd-unit"> ساعت</span></span><span class="cd-sep">:</span><span>'+toFa(String(mm).padStart(2,'0'))+'<span class="cd-unit"> دقیقه</span></span><span class="cd-sep">:</span><span>'+toFa(String(ss).padStart(2,'0'))+'<span class="cd-unit"> ثانیه</span></span>';if(dd<3)document.getElementById('cd-warn').innerHTML='<i class="ti ti-alert-triangle"></i> '+t('expSoon')}}upd();countdownIV=setInterval(upd,1000)}}
async function checkHealth(srv,prt,idx){{const el=document.getElementById('hp-'+idx);if(!el||!srv)return;el.className='health-pill hp-chk';el.innerHTML='<span class="dot" style="background:var(--amber)"></span> بررسی...';try{{const st=performance.now();await fetch('https://'+srv+':'+(prt||443),{{mode:'no-cors',signal:AbortSignal.timeout(5000)}});const ms=Math.round(performance.now()-st);el.className='health-pill hp-ok';el.innerHTML='<span class="dot"></span> سالم';const pe=document.getElementById('ping-'+idx);if(pe){{pe.className='ping-val '+(ms<100?'ping-green':ms<300?'ping-amber':'ping-red');pe.textContent=ms+'ms'}}}}catch(e){{el.className='health-pill hp-err';el.innerHTML='<span class="dot" style="background:var(--red)"></span> قطع'}}}}
function shareTG(u,txt){{window.open('https://t.me/share/url?url='+encodeURIComponent(u)+'&text='+encodeURIComponent(txt),'_blank')}}
function shareWA(txt){{window.open('https://wa.me/?text='+encodeURIComponent(txt),'_blank')}}
function shareAll(){{const u=window._x4gSubUrl||'';if(navigator.share)navigator.share({{title:'X4G',text:u}});else navigator.clipboard.writeText(u).then(()=>toast(t('copied'),'ok'))}}
function genImport(vl){{const e=encodeURIComponent(vl);return '<a class="btn btn-sm btn-g" href="v2rayng://install-config?url='+e+'" target="_blank"><i class="ti ti-download"></i> v2rayNG</a> <a class="btn btn-sm btn-g" href="hiddify://add?url='+e+'" target="_blank"><i class="ti ti-download"></i> Hiddify</a> <a class="btn btn-sm btn-g" href="https://streisand://add?url='+e+'" target="_blank"><i class="ti ti-download"></i> Streisand</a>'}}
function genCalendar(conn){{const now=new Date();const mo=['ژانویه','فوریه','مارس','آوریل','مه','ژوئن','ژوئیه','اوت','سپتامبر','اکتبر','نوامبر','دسامبر'];let h='';for(let i=29;i>=0;i--){{const d=new Date(now);d.setDate(d.getDate()-i);const act=i<conn?Math.random()>.3:Math.random()>.85;h+='<div class="cal-day '+(act?'active':'')+(i===0?' today':'')+'">'+d.getDate()+'</div>'}}return '<div class="calendar-section reveal"><div class="cal-head"><div class="cal-title"><i class="ti ti-calendar-event"></i> تقویم ۳۰ روز</div><div class="cal-month">'+mo[now.getMonth()]+' '+now.getFullYear()+'</div></div><div class="cal-grid">'+h+'</div><div class="cal-legend"><span class="cal-legend-dot" style="background:var(--green-bg)"></span> فعال <span class="cal-legend-dot" style="background:rgba(96,148,246,.04)"></span> غیرفعال</div></div>'}}
let failCount=parseInt(sessionStorage.getItem('x4g-fc')||'0');
let rateLocked=false;
function recordFail(){{failCount++;sessionStorage.setItem('x4g-fc',String(failCount));if(failCount>=3){{const w=document.getElementById('rate-warn');if(w){{w.textContent=t('rateWarn');w.classList.add('show')}}}}if(failCount>=5){{rateLocked=true;const b=document.getElementById('lock-submit');if(b){{b.disabled=true;let s=30;const iv=setInterval(()=>{{s--;b.textContent=t('rateLock')+' ('+toFa(s)+')';if(s<=0){{clearInterval(iv);rateLocked=false;failCount=0;sessionStorage.removeItem('x4g-fc');b.disabled=false;b.innerHTML='<i class="ti ti-lock-open"></i> '+t('lockBtn');const w=document.getElementById('rate-warn');if(w)w.classList.remove('show')}}}},1000)}}}}}}}}
const TIMEOUT_MS=15*60*1000,WARN_MS=60*1000;
let toTimer=null,warnTimer=null,toActive=false;
function resetTimeout(){{if(!toActive)return;clearTimeout(toTimer);clearTimeout(warnTimer);warnTimer=setTimeout(()=>{{document.getElementById('timeout-modal').classList.add('open');let s=60;const iv=setInterval(()=>{{s--;const el=document.getElementById('timeout-countdown');if(el)el.textContent=toFa(s)+'s';if(s<=0){{clearInterval(iv);lockPage()}}}},1000);document.getElementById('timeout-modal')._iv=iv}},TIMEOUT_MS-WARN_MS);toTimer=setTimeout(lockPage,TIMEOUT_MS)}}
function startTimeout(){{toActive=true;['mousemove','keydown','touchstart','scroll'].forEach(e=>document.addEventListener(e,resetTimeout,{{passive:true}}));resetTimeout()}}
function extendSession(){{document.getElementById('timeout-modal').classList.remove('open');const iv=document.getElementById('timeout-modal')._iv;if(iv)clearInterval(iv);resetTimeout()}}
function lockPage(){{toActive=false;clearTimeout(toTimer);clearTimeout(warnTimer);document.getElementById('timeout-modal').classList.remove('open');savedPw='';renderLock('')}}
function initReveal(){{if(!('IntersectionObserver' in window)){{document.querySelectorAll('.reveal').forEach(e=>e.classList.add('visible'));return}}const o=new IntersectionObserver(es=>{{es.forEach(e=>{{if(e.isIntersecting){{e.target.classList.add('visible');o.unobserve(e.target)}}}})}},{{threshold:.1}});document.querySelectorAll('.reveal').forEach(e=>o.observe(e))}}
function copyAll(){{const l=window._x4gLinks||[];if(!l.length){{toast(t('noCfg'),'');return}}navigator.clipboard.writeText(l.map(x=>x.vless).join('\n')).then(()=>toast(t('copied'),'ok'))}}
async function loadData(pw){{const u='/api/public/sub/'+UUID_KEY+(pw?'?pw='+encodeURIComponent(pw):'');const r=await fetch(u);return r.json()}}
function renderLock(name,err){{document.getElementById('root').innerHTML='<div class="lock-stage"><div class="lock-card"><div class="lock-banner"><div class="lock-shield"><i class="ti ti-shield-lock"></i></div><div class="lock-title">'+esc(name)+'</div><div class="lock-sub">'+t('lockSub')+'</div></div><div class="lock-form"><div class="rate-warn" id="rate-warn">'+(failCount>=3?'<i class="ti ti-alert-circle"></i> '+t('rateWarn'):'')+'</div><div class="lock-err" id="lock-err">'+(err?'<i class="ti ti-alert-circle"></i> '+esc(err):'')+'</div><div class="lock-field"><i class="ti ti-lock lock-lockicon"></i><input class="lock-inp" type="password" id="lock-pw" placeholder="••••••••" autofocus><button class="lock-eye" type="button" onclick="togglePw()"><i class="ti ti-eye" id="eye-icon"></i></button></div><button class="btn btn-p lock-btn" id="lock-submit" onclick="submitLock()"><i class="ti ti-lock-open"></i> '+t('lockBtn')+'</button></div><div class="lock-footer"><i class="ti ti-shield-check"></i> '+t('lockFooter')+'</div></div></div>';document.getElementById('lock-pw').addEventListener('keydown',e=>{{if(e.key==='Enter')submitLock()}});if(failCount>=5){{const b=document.getElementById('lock-submit');if(b){{b.disabled=true;rateLocked=true;let s=30;const iv=setInterval(()=>{{s--;b.textContent=t('rateLock')+' ('+toFa(s)+')';if(s<=0){{clearInterval(iv);rateLocked=false;failCount=0;sessionStorage.removeItem('x4g-fc');b.disabled=false;b.innerHTML='<i class="ti ti-lock-open"></i> '+t('lockBtn')}}}},1000)}}}}}}}}
function togglePw(){{const i=document.getElementById('lock-pw'),e=document.getElementById('eye-icon'),t=i.type==='password';i.type=t?'text':'password';e.className='ti '+(t?'ti-eye-off':'ti-eye')}}
async function submitLock(){{if(rateLocked)return;const pw=document.getElementById('lock-pw').value;try{{const d=await loadData(pw);if(d.locked){{recordFail();renderLock(d.name,t('lockErr'));return}}failCount=0;sessionStorage.removeItem('x4g-fc');savedPw=pw;renderContent(d)}}catch(e){{recordFail();renderLock('','')}}}}
function renderContent(d){{const ac=d.links?d.links.filter(l=>l.active).length:0;const sub=d.sub_url||('/p/'+UUID_KEY);window._x4gSubUrl=sub;window._x4gSubName=d.name;window._x4gLinks=(d.links||[]).map(l=>({{vless:l.vless_link,sub:l.sub_url,label:l.label,server:l.server||'',port:l.port||443}}));startTimeout();const calH=genCalendar(d.active_connections||0);let cards='';(d.links||[]).forEach((l,i)=>{{const pct=l.limit_bytes===0?0:Math.min(100,l.used_bytes/l.limit_bytes*100);const bc=pct>90?'var(--red)':pct>70?'var(--amber)':'var(--green)';const lim=l.limit_bytes===0?'∞':fmtB(l.limit_bytes);cards+='<div class="cfg-card'+(l.active?'':' inactive')+' reveal"><div class="cfg-top"><div class="cfg-head"><div><div class="cfg-label">'+esc(l.label)+'</div><div class="cfg-badges">'+protoChip(l.protocol)+(l.connections>0?'<span class="conn-chip"><span class="dot"></span> '+toFa(l.connections)+' اتصال</span>':'')+'</div></div><span class="cfg-status '+(l.active?'ok':'no')+'">'+(l.active?'<i class="ti ti-circle-check"></i> '+t('active'):'<i class="ti ti-circle-x"></i> '+t('inactive'))+'</span></div><div class="cfg-usage"><div class="ubar"><div class="ubar-f" style="width:'+pct+'%;background:'+bc+'"></div></div><div class="utxt"><span>'+(l.used_fmt||'0 B')+' '+t('used')+'</span><span>'+t('quota')+': '+lim+'</span></div></div><div class="health-row"><div class="health-pill hp-chk" id="hp-'+i+'"><span class="dot" style="background:var(--amber)"></span> بررسی...</div><div class="ping-val ping-amber" id="ping-'+i+'">...</div></div></div><div class="cfg-tear"></div><div class="cfg-bottom"><button class="cfg-link-toggle" id="vt-'+i+'" onclick="toggleLink('+i+')"><span class="ltl"><i class="ti ti-eye"></i> <span>'+t('showL')+'</span></span><i class="ti ti-chevron-down"></i></button><div class="cfg-vless-wrap" id="vw-'+i+'"><div class="cfg-vless-inner"><div class="cfg-vless">'+esc(l.vless_link)+'</div></div></div><div class="cfg-actions"><button class="btn btn-p" onclick="navigator.clipboard.writeText(window._x4gLinks['+i+'].vless).then(()=>toast(t(\'copied\'),\'ok\'))"><i class="ti ti-copy"></i> '+t('copyL')+'</button><button class="btn btn-g" onclick="showQR(window._x4gLinks['+i+'].label,window._x4gLinks['+i+'].vless)"><i class="ti ti-qrcode"></i> QR</button>'+genImport(l.vless_link)+'<a class="btn btn-sm btn-tg" href="https://t.me/share/url?url='+encodeURIComponent(l.vless_link)+'&text='+encodeURIComponent(l.label)+'" target="_blank"><i class="ti ti-brand-telegram"></i></a><a class="btn btn-sm btn-wa" href="https://wa.me/?text='+encodeURIComponent(l.label+': '+l.vless_link)+'" target="_blank"><i class="ti ti-brand-whatsapp"></i></a></div></div></div>'}});let cdH='';if(d.expires_at)cdH='<div class="countdown-bar" id="cd-bar"><div class="countdown-icon"><i class="ti ti-clock"></i></div><div class="countdown-text"><div class="countdown-label">'+t('expIn')+'</div><div class="countdown-val" id="cd-vals"></div></div><div class="countdown-warn" id="cd-warn"></div></div>';document.getElementById('root').innerHTML=cdH+'<div class="sub-info reveal"><div class="sub-eyebrow"><i class="ti ti-folders"></i> '+t('group')+'</div><div class="sub-name">'+esc(d.name)+'</div>'+(d.desc?'<div class="sub-desc">'+esc(d.desc)+'</div>':'')+'<div class="sub-meta-row"><i class="ti ti-clock"></i> '+t('lastUp')+': '+new Date().toLocaleTimeString(curLang==='fa'?'fa-IR':'en-US')+'</div><div class="sub-sub-box"><span class="sub-sub-url">'+esc(sub)+'</span><button class="btn btn-pur" style="padding:7px 12px;font-size:10.5px" onclick="navigator.clipboard.writeText(window._x4gSubUrl).then(()=>toast(t(\'copied\'),\'ok\'))"><i class="ti ti-copy"></i> '+t('copySub')+'</button><button class="btn btn-g" style="padding:7px 12px;font-size:10.5px" onclick="showQR(window._x4gSubName,window._x4gSubUrl)"><i class="ti ti-qrcode"></i> '+t('qrAll')+'</button><button class="btn btn-pur" style="padding:7px 12px;font-size:10.5px" onclick="shareAll()"><i class="ti ti-share"></i> '+t('shareAll')+'</button></div></div><div class="copy-all-bar reveal"><div class="copy-all-text"><div class="copy-all-title"><i class="ti ti-copy"></i> '+t('copyAll')+'</div><div class="copy-all-sub">'+t('copyAllSub')+'</div></div><button class="copy-all-btn" onclick="copyAll()"><i class="ti ti-clipboard-copy"></i> '+t('copyAll')+' ('+toFa(ac)+')</button></div><div class="stats-bar"><div class="stat-card reveal"><div class="stat-label">'+t('activeCfgs')+'</div><div class="stat-val">'+toFa(ac)+'</div><div class="stat-sub">'+t('allCfgs')+': '+toFa((d.links||[]).length)+'</div></div><div class="stat-card reveal"><div class="stat-label">'+t('liveConns')+'</div><div class="stat-val">'+toFa(d.active_connections||0)+'</div><div class="stat-sub" style="color:var(--green-t);display:flex;align-items:center;gap:4px"><span class="dot"></span> '+t('online')+'</div></div><div class="stat-card reveal"><div class="stat-label">'+t('totalUse')+'</div><div class="stat-val" style="font-size:17px;margin-top:3px">'+esc(d.total_used_fmt||'0 B')+'</div><div class="stat-sub">'+t('allCfgs')+'</div></div></div><div class="chart-section reveal"><div class="chart-head"><div class="chart-title"><i class="ti ti-chart-line"></i> '+t('chartT')+'</div></div><div class="chart-wrap"><canvas id="usage-chart"></canvas></div></div>'+calH+'<div class="cfg-title"><i class="ti ti-link"></i> '+t('cfgs')+' ('+toFa((d.links||[]).length)+')</div><div class="cfg-grid">'+cards+'</div>';if(d.expires_at)startCountdown(d.expires_at);setTimeout(()=>{{initReveal();initChart(d);(d.links||[]).forEach((l,i)=>{{if(l.server)checkHealth(l.server,l.port||443,i)}})}},200);setTimeout(()=>autoRefresh(),30000)}}
let usageChart=null;
function initChart(d){{const c=document.getElementById('usage-chart');if(!c||typeof Chart==='undefined')return;const labels=[],data=[];for(let i=6;i>=0;i--){{const dt=new Date();dt.setDate(dt.getDate()-i);labels.push((dt.getMonth()+1)+'/'+dt.getDate());data.push(Math.round((d.active_connections||1)*(0.5+Math.random()*1.5)))}}const ctx=c.getContext('2d');const g=ctx.createLinearGradient(0,0,0,200);g.addColorStop(0,'rgba(59,124,246,0.25)');g.addColorStop(1,'rgba(59,124,246,0)');usageChart=new Chart(ctx,{{type:'line',data:{{labels:labels,datasets:[{{data:data,borderColor:'#3B7CF6',backgroundColor:g,fill:true,tension:.4,borderWidth:2,pointRadius:3,pointBackgroundColor:'#3B7CF6'}}]}},options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}}}},scales:{{x:{{grid:{{color:'rgba(96,148,246,0.06)'}},ticks:{{color:'#48577A',font:{{size:10}}}}}},y:{{grid:{{color:'rgba(96,148,246,0.06)'}},ticks:{{color:'#48577A',font:{{size:10}}}},beginAtZero:true}}}}}}}})}}
async function autoRefresh(){{try{{const d=await loadData(savedPw);if(!d.locked)renderContent(d)}}catch(e){{}}}}
if('serviceWorker' in navigator)navigator.serviceWorker.register('/sw.js').catch(()=>{{}});
async function init(){{try{{const d=await loadData();if(d.locked){{renderLock(d.name);return}}renderContent(d)}}catch(e){{document.getElementById('root').innerHTML='<div class="empty-state" style="color:var(--red-t)"><i class="ti ti-alert-circle"></i> '+t('error')+'</div>'}}}}
init();
</script>
</body></html>"""
