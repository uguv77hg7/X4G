...
@media(max-width:600px){.topnav{padding:0 12px}.tn-brand{font-size:14px}.stats-row{grid-template-columns:1fr}.traf-hero{grid-template-columns:1fr}.conn-hero{grid-template-columns:1fr}.proto-cards{grid-template-columns:1fr}.cp-footer{flex-direction:column;align-items:stretch}.cp-submit-btn{justify-content:center}}
</style>
</head>
<body>
...
document.addEventListener('DOMContentLoaded',async()=>{initParticles();await checkAuth();initCharts();document.getElementById('set-host').textContent=location.host;fetchStats();loadLinks();setInterval(fetchStats,4000);setInterval(()=>{if(document.getElementById('pg-links').classList.contains('on'))loadLinks();if(document.getElementById('pg-connections').classList.contains('on'))loadConns();if(document.getElementById('pg-logs').classList.contains('on'))loadActivity();if(document.getElementById('pg-cfgdash').classList.contains('on'))loadCfgDash();},5000);});
</script>
</body></html>"""
